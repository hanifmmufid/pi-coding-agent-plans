...
export default function visionToolExtension(pi: ExtensionAPI) {
  ...
  pi.registerCommand("vision", {
    ...
    handler: async (args, ctx) => {
      ...
      // /vision list — tampilkan semua model vision-capable & biarkan user pilih
      if (subcommand === "list" || subcommand === "models") {
        const vModels = ctx.modelRegistry.getAvailable().filter((m) => m.input?.includes("image"));
        if (vModels.length === 0) {
          ctx.ui.notify("No vision-capable models available (input: text+image). Check models.json.", "warning");
          return;
        }
        const current = config.model ?? "";
        const labels = vModels.map((m) =>
          `${m.id} (${m.provider})${m.id === current ? "  ← aktif" : ""}`,
        );
        const choice = await ctx.ui.select("Pilih model vision:", labels, { timeout: 120_000 });
        if (!choice) {
        ...
        if (idx === -1) return;
        const vm = vModels[idx];
        config.provider = vm.provider;
        config.model = vm.id;
        saveConfigFile();
        persistConfig();
        updateStatus(ctx);
        ctx.ui.notify(`Vision model set to "${config.provider}/${config.model}" (saved to ${CONFIG_PATH})`, "info");
        return;
      }
      ...
      if (subcommand === "config" || subcommand === "cfg") {
        ...
        if (setting === "model") {
          ...
          // Tanpa nilai → tampilkan selector & set langsung.
          const visionModels = ctx.modelRegistry.getAvailable().filter((m) => m.input?.includes("image"));
          if (visionModels.length === 0) {
            ctx.ui.notify(`Current model: ${config.model ?? "(not set)"} (no vision-capable models available)`, "info");
            return;
          }
          const current = config.model ?? "";
